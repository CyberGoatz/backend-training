...
public class TrainingConsoleCommandsDao extends AbstractElasticClientDAO {
   ...
   */
  @Autowired
  public TrainingConsoleCommandsDao(
   ...
   */
  public List<Map<String, Object>> findAllConsoleCommands(
      String index, List<String> filterCommands, CommandType commandType)
    ...
    if (filterCommands != null && !filterCommands.isEmpty()) {
      searchSourceBuilder.query(
          QueryBuilders.regexpQuery(
              AbstractCrczpElasticTermQueryFields.CRCZP_ELASTICSEARCH_COMMAND,
              StringUtils.collectionToDelimitedString(filterCommands, "|")));
    }
    if (commandType != null) {
      String commandTypeString = commandType.toString().toLowerCase() + "-command";
      searchSourceBuilder.query(
          QueryBuilders.regexpQuery(
              AbstractCrczpElasticTermQueryFields.CRCZP_ELASTICSEARCH_COMMAND_TYPE,
              commandTypeString));
    }
   ...
   *
   * <p>Elasticserach query: GET crczp.logs.console*.sandbox={sandboxId}/_search { "query": {
   * "range": { "timestamp_str": { "gte": {from} "lte": {to} } } } }
   *
   ...
   * @throws IOException the io exception
   */
  public List<Map<String, Object>> findAllConsoleCommandsBySandboxAndTimeRange(
      String index, Long from, Long to, List<String> filterCommands, CommandType commandType)
      throws ElasticsearchTrainingDataLayerException, IOException {
    SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
    searchSourceBuilder.sort(
        AbstractCrczpElasticTermQueryFields.CRCZP_ELASTICSEARCH_TIMESTAMP_STR, SortOrder.ASC);
  ...
  private List<Map<String, Object>> handleElasticsearchResponse(SearchResponse response)
    ...
    if (response != null) {
      ...
      throw new ElasticsearchTrainingDataLayerException(
          "Client could not connect to Elasticsearch. Please, restart Elasticsearch service.");
    }
    return events;
  }

  private List<List<Map<String, Object>>> resolveTimeRangeAggregation(
      SearchResponse searchResponse) {
    Aggregations aggregations = searchResponse.getAggregations();
    ParsedRange commandsByTimeRanges = aggregations.get("timestamp_ranges");
    Collection<Range.Bucket> timestampRanges =
        (Collection<Range.Bucket>) commandsByTimeRanges.getBuckets();
  ...
  private List<Map<String, Object>> resolveBucketTopHits(Range.Bucket rangeBucket)
      throws ElasticsearchTrainingDataLayerException {
