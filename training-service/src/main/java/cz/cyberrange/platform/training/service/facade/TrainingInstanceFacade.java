...
public class TrainingInstanceFacade {
  ...
  public TrainingInstanceDTO findById(Long id) {
    TrainingInstanceDTO trainingInstanceDTO =
        trainingInstanceMapper.mapToDTO(trainingInstanceService.findByIdIncludingDefinition(id));
    List<AbstractLevel> levels =
        trainingDefinitionService.findAllLevelsFromDefinition(
            trainingInstanceDTO.getTrainingDefinition().getId());
    trainingInstanceDTO
        ...
        .setHasReferenceSolution(
            levels.stream()
                ...
                .anyMatch(
                    trainingLevel ->
                        !((TrainingLevel) trainingLevel).getReferenceSolution().isEmpty()));
