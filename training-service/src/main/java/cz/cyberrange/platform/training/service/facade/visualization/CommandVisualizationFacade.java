...
public class CommandVisualizationFacade {
  ...
  @Autowired
  public CommandVisualizationFacade(
  ...
  public Object getAggregatedCommandsForOrganizer(
      List<Long> runIds, boolean correct, List<MistakeType> mistakeTypes) {
    if (correct) {
  ...
  public Object getAggregatedCommandsForTrainee(
      Long runId, boolean correct, List<MistakeType> mistakeTypes) {
    if (correct) {
      ...
      this.checkIfTrainingRunIsFinished(runId);
      return trainingFeedbackApiService.getAggregatedIncorrectCommands(
          List.of(runId), mistakeTypes);
    }
  }
  ...
  public List<TrainingRunDTO> findTrainingRunsByTrainingInstance(Long instanceId) {
    Page<TrainingRun> trainingRuns =
        trainingInstanceService.findFinishedTrainingRunsByTrainingInstance(
            instanceId, PageRequest.of(0, 999));
    List<TrainingRunDTO> trainingRunDTOs =
        trainingRunMapper.mapToListDTO(trainingRuns.getContent());
    ...
    if (trainingRun.getState() != TRState.FINISHED) {
      throw new EntityConflictException(
          new EntityErrorDetail(
              ...
              runId.getClass(),
              runId,
              "Training run has not been finished yet."));
    }
  }

  private void checkIfTrainingRunsAreFinished(List<Long> runIds) {
    QTrainingRun trainingRun = QTrainingRun.trainingRun;
    BooleanExpression queryRunningTrainingRuns =
        trainingRun.id.in(runIds).and(trainingRun.state.eq(TRState.RUNNING));
    Page<TrainingRun> runningTrainingRuns =
        trainingRunService.findAll(queryRunningTrainingRuns, PageRequest.of(0, 1));
    if (!runningTrainingRuns.isEmpty()) {
      throw new EntityConflictException(
          new EntityErrorDetail(
              TrainingRun.class,
              "id",
              runningTrainingRuns.getContent().get(0).getId().getClass(),
              runningTrainingRuns.getContent().get(0).getId(),
              "Training run has not been finished yet."));
    }
  }
  ...
  public Map<Long, List<CommandDTO>> getAllCommandsByTrainingInstance(Long trainingInstanceId) {
    Set<TrainingRun> trainingRunSet =
    ...
    trainingRunSet.stream()
        .map(TrainingRun::getId)
        .forEach(runId -> commandsByTrainingRun.put(runId, getAllCommandsByTrainingRun(runId)));
  ...
  public List<CommandDTO> getAllCommandsByTrainingRun(Long runId) {
    ...
    List<Map<String, Object>> openSearchCommands =
        opensearchApiService.findAllConsoleCommandsBySandbox(sandboxId);

    return openSearchCommandsToCommandDTOlist(openSearchCommands, trainingRun.getStartTime());
  }

  private List<CommandDTO> openSearchCommandsToCommandDTOlist(
      List<Map<String, Object>> openSearchCommands, LocalDateTime runStartTime) {
    List<CommandDTO> commandDTOS = new ArrayList<>(openSearchCommands.size());
    openSearchCommands.stream()
        .map(
            openSearchCommand ->
                objectMapper.convertValue(openSearchCommand, OpenSearchCommand.class))
        .forEach(command -> commandDTOS.add(openSearchCommandToCommandDTO(command, runStartTime)));
    return commandDTOS;
  }

  private CommandDTO openSearchCommandToCommandDTO(
      OpenSearchCommand openSearchCommand, LocalDateTime runStartTime) {
    String[] commandSplit = openSearchCommand.getCmd().split(" ", 2);
