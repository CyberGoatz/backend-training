...
public class TrainingDefinitionFacade {
  ...
  private void updateReferenceSolution(Long definitionId) {
    ...
    for (TrainingLevel level : this.trainingDefinitionService.getAllTrainingLevels(definitionId)) {
      referenceSolution.add(
          new LevelReferenceSolutionDTO(
              level.getId(),
              level.getOrder(),
              new ArrayList<>(
                  ReferenceSolutionNodeMapper.INSTANCE.mapToSetDTO(level.getReferenceSolution()))));
   ...
   * @return true if at least one of the training levels has reference solution defined, false
   *     otherwise.
   */
  @IsTrainee
  @TransactionalRO
