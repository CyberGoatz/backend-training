...
public class TrainingInstancesIT {
  ...
  @Autowired private TrainingInstancesRestController trainingInstancesRestController;
  @Autowired private TrainingInstanceMapperImpl trainingInstanceMapper;
  ...
  private TrainingInstance futureTrainingInstance,
      notConcludedTrainingInstance,
      finishedTrainingInstance;
  ...
  public void findAllTrainingInstancesAsAdmin() throws Exception {
    ...
    MockHttpServletResponse result =
        mvc.perform(get("/training-instances"))
            .andExpect(status().isOk())
            .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
    PageResultResource<TrainingInstanceDTO> trainingInstancesPage =
        mapper.readValue(
            convertJsonBytesToString(result.getContentAsString()),
            new TypeReference<PageResultResource<TrainingInstanceDTO>>() {});
    assertTrue(
        trainingInstancesPage
            .getContent()
            .contains(trainingInstanceMapper.mapToDTO(notConcludedTrainingInstance)));
    assertTrue(
        trainingInstancesPage
            .getContent()
            .contains(trainingInstanceMapper.mapToDTO(futureTrainingInstance)));
  }
  ...
  public void findTrainingInstanceByIdNotFound() throws Exception {
    MockHttpServletResponse response =
        mvc.perform(get("/training-instances/{id}", 100L))
  ...
  public void createTrainingInstance() throws Exception {
    ...
    assertEquals(
        newInstanceDTO,
        mapper.readValue(
            convertJsonBytesToString(result.getContentAsString()), TrainingInstanceDTO.class));
  }
  ...
  public void createInvalidTrainingInstance() throws Exception {
    Exception ex =
        mvc.perform(
                post("/training-instances")
                    .content(convertObjectToJsonBytes(new TrainingInstanceCreateDTO()))
                    .contentType(MediaType.APPLICATION_JSON_VALUE))
  ...
  public void updateTrainingInstance() throws Exception {
    ...
    Optional<TrainingInstance> newInstance =
        trainingInstanceRepository.findById(futureTrainingInstance.getId());
    assertTrue(newInstance.isPresent());
  ...
  public void updateRunningTrainingInstanceDifferentTrainingDefinition() throws Exception {
    ...
    assertEntityDetailError(
        ...
        TrainingInstance.class,
        "id",
        notConcludedTrainingInstance.getId().toString(),
  ...
  public void updateRunningTrainingInstanceChangeStartTime() throws Exception {
    ...
    trainingInstanceUpdateDTO.setAccessToken("preff");
    trainingInstanceUpdateDTO.setTrainingDefinitionId(
        notConcludedTrainingInstance.getTrainingDefinition().getId());
    mockSpringSecurityContextForGet(List.of(RoleType.ROLE_TRAINING_ORGANIZER.name()));
    MockHttpServletResponse response =
        mvc.perform(
                put("/training-instances")
  ...
  public void updateTrainingInstanceWrongStartTime() throws Exception {
    ...
    assertEntityDetailError(
        ...
        TrainingInstance.class,
        "id",
        futureTrainingInstance.getId().toString(),
  ...
  public void deleteTrainingInstance() throws Exception {
    ...
    mvc.perform(delete("/training-instances/{id}", tI.getId())).andExpect(status().isOk());
  ...
  public void deleteFinishedTrainingInstanceWithUnassignedPool() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(delete("/training-instances/{id}", finishedTrainingInstance.getId()))
            .andExpect(status().isConflict())
            .andReturn()
            .getResponse();
    ApiEntityError error =
    ...
    assertEntityDetailError(
        ...
        TrainingInstance.class,
        "id",
        finishedTrainingInstance.getId().toString(),
  ...
  public void findAllTrainingRunsByTrainingInstanceIdWithNonexistentInstance() throws Exception {
    MockHttpServletResponse response =
        mvc.perform(get("/training-instances/{instanceId}/training-runs", 100L))
            .andExpect(status().isNotFound())
            .andReturn()
            .getResponse();
    ApiEntityError error =
