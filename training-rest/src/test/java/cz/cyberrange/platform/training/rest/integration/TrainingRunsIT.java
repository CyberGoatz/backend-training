...
@Transactional
@TestPropertySource(properties = {"sandbox-service.uri=http://localhost:8080"})
public class TrainingRunsIT {

  private MockMvc mvc;
  @Autowired private ApplicationContext applicationContext;

  @Autowired private TrainingRunsRestController trainingRunsRestController;
  ...
  @Autowired private OpenSearchApiService opensearchApiServiceMock;
  ...
  public void init() {
    ...
    isCorrectAnswerDTO.setRemainingAttempts(
        trainingLevel1.getIncorrectAnswerLimit() - trainingRun1.getIncorrectAnswerCount());
  ...
  public void findTrainingRunById() throws Exception {
    ...
    given(userManagementExchangeFunction.exchange(any(ClientRequest.class)))
        .willReturn(buildMockResponse(userRefDTO1));
    MockHttpServletResponse result =
        mvc.perform(get("/training-runs/{id}", trainingRun.getId()))
    ...
    TrainingRunByIdDTO resultDTO =
        mapper.readValue(
            convertJsonBytesToString(result.getContentAsString()), TrainingRunByIdDTO.class);
  ...
  public void findTrainingRunByIdNotFound() throws Exception {
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{id}", 100L))
  ...
  public void findAllTrainingRuns() throws Exception {
    ...
    MockHttpServletResponse result =
        mvc.perform(get("/training-runs"))
            .andExpect(status().isOk())
            .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
    PageResultResource<TrainingRunDTO> trainingRunsPage =
        mapper.readValue(
            convertJsonBytesToString(result.getContentAsString()),
            new TypeReference<PageResultResource<TrainingRunDTO>>() {});
  ...
  public void accessTrainingRun() throws Exception {
    given(userManagementExchangeFunction.exchange(any(ClientRequest.class)))
        .willReturn(buildMockResponse(userRefDTO2));
    given(sandboxManagementExchangeFunction.exchange(any(ClientRequest.class)))
        .willReturn(buildMockResponse(sandboxInfo));
    ...
    MockHttpServletResponse response =
        mvc.perform(post("/training-runs").param("accessToken", trainingInstance.getAccessToken()))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    AccessTrainingRunDTO trainingRunDTO =
        convertJsonBytesToObject(response.getContentAsString(), AccessTrainingRunDTO.class);
    Optional<TrainingRun> accessedTrainingRun =
        trainingRunRepository.findById(trainingRunDTO.getTrainingRunID());
  ...
  public void accessTrainingRunWithTrainingInstanceWithoutSandboxes() throws Exception {
    ...
    assertEntityDetailError(
        ...
        "id",
        trainingInstance.getId().toString(),
  ...
  public void accessTrainingRunDefinitionWithoutLevels() throws Exception {
    ...
    assertEntityDetailError(
        ...
        "id",
        trainingInstance.getTrainingDefinition().getId().toString(),
  ...
  public void accessTrainingRunNoAvailableSandbox() throws Exception {
    ...
    String url =
        "http://localhost:8080" + "/pools/" + trainingInstance.getPoolId() + "/sandboxes/unlocked/";
    UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(url);
    builder.queryParam("page", 1);
    builder.queryParam("page_size", 1000);
    given(userManagementExchangeFunction.exchange(any(ClientRequest.class)))
    ...
    willThrow(
            new CustomWebClientException(
                HttpStatus.CONFLICT, PythonApiError.of("No unlocked sandbox.")))
        .given(sandboxManagementExchangeFunction)
  ...
  public void accessTrainingRunAccessTokenNotFound() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(post("/training-runs").param("accessToken", "notFoundToken"))
            .andExpect(status().isNotFound())
            .andReturn()
            .getResponse();
    ApiEntityError error =
  ...
  public void getAllAccessedTrainingRuns() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/accessible"))
            ...
            .andReturn()
            .getResponse();
    PageResultResource<AccessedTrainingRunDTO> responseTrainingRunDTOPage =
        mapper.readValue(
            convertJsonBytesToString(response.getContentAsString()),
            new TypeReference<PageResultResource<AccessedTrainingRunDTO>>() {});
  ...
  public void getNextLevel() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/next-levels", trainingRun1.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
            .andReturn()
            .getResponse();
    InfoLevelDTO infoLevelDTO =
        mapper.readValue(
            convertJsonBytesToString(response.getContentAsString()), InfoLevelDTO.class);
  ...
  public void getNextLevelTrainingLevel() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/next-levels", trainingRun1.getId()))
            ...
            .andReturn()
            .getResponse();
    TrainingLevelViewDTO trainingLevelDTO =
        mapper.readValue(
            convertJsonBytesToString(response.getContentAsString()), TrainingLevelViewDTO.class);
  ...
  public void getNextLevelNoLevelAnswered() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/next-levels", trainingRun2.getId()))
            .andExpect(status().isConflict())
            .andReturn()
            .getResponse();
    ApiEntityError error =
  ...
  public void getSolutionNotTrainingLevel() throws Exception {
    ...
    assertTrue(
        ex.getMessage()
            .contains("Current level is not training level and does not have solution."));
  }

  @Test
  public void getSolutionLevelNotFound() throws Exception {
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/solutions", nonExistentTrainingRunId))
            .andExpect(status().isNotFound())
            .andReturn()
            .getResponse();
    ApiEntityError error =
    ...
    assertEntityDetailError(
        ...
        TrainingRun.class,
        "id",
        nonExistentTrainingRunId.toString(),
        "Entity TrainingRun (id: 100) not found.");
  }
  ...
  public void getHint() throws Exception {
    ...
    MockHttpServletResponse response =
        mvc.perform(
                get("/training-runs/{runId}/hints/{hintId}", trainingRun1.getId(), hint.getId()))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    assertEquals(
        hintMapper.mapToDTO(hint),
        mapper.readValue(convertJsonBytesToString(response.getContentAsString()), HintDTO.class));
    assertTrue(
        trainingRun1
            .getHintInfoList()
            .contains(
                new HintInfo(
                    trainingRun1.getCurrentLevel().getId(),
                    hint.getId(),
                    hint.getTitle(),
                    hint.getContent(),
                    hint.getOrder())));
  ...
  public void getHintTrainingRunNotFound() throws Exception {
    trainingRunRepository.save(trainingRun1);
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/hints/{hintId}", nonExistentTrainingRunId, 1L))
            .andExpect(status().isNotFound())
            .andReturn()
            .getResponse();
    ApiEntityError error =
  ...
  public void getHintWrongLevelType() throws Exception {
    ...
    Exception exception =
        mvc.perform(
                get("/training-runs/{runId}/hints/{hintId}", trainingRun2.getId(), hint.getId()))
            .andExpect(status().isBadRequest())
            .andReturn()
            .getResolvedException();
  ...
  public void isCorrectAnswer() throws Exception {
    ...
    assertEquals(
        isCorrectAnswerDTO,
        mapper.readValue(response.getContentAsString(), IsCorrectAnswerDTO.class));
  ...
  public void isCorrectAnswerNoGameLevel() throws Exception {
    ...
    Exception ex =
        mvc.perform(
                post("/training-runs/{runId}/is-correct-answer", trainingRun2.getId())
                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                    .content(convertObjectToJsonBytes(invalidAnswerDTO)))
            .andExpect(status().isBadRequest())
            .andReturn()
  ...
  public void resumeTrainingRunWithoutTakenSolutionAndHints() throws Exception {
    trainingRunRepository.save(trainingRun1);
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/resumption", trainingRun1.getId()))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();
    AccessTrainingRunDTO trainingRunDTO =
  ...
  public void resumeTrainingRunWithTakenHints() throws Exception {
    ...
    Optional<TrainingRun> trainingRun =
        trainingRunRepository.findById(accessTrainingRunDTO.getTrainingRunID());
  ...
  public void resumeFinishedTrainingRun() throws Exception {
    ...
    trainingRunRepository.save(trainingRun2);
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/resumption", trainingRun2.getId()))
            .andExpect(status().isConflict())
            .andReturn()
            .getResponse();
    ApiEntityError error =
  ...
  public void resumeTrainingRunNotFound() throws Exception {
    MockHttpServletResponse response =
        mvc.perform(get("/training-runs/{runId}/resumption", nonExistentTrainingRunId))
            .andExpect(status().isNotFound())
            .andReturn()
            .getResponse();
    ApiEntityError error =
  ...
  public void finishTrainingRun() throws Exception {
    trainingRun1.setCurrentLevel(infoLevel1);
    trainingRunRepository.save(trainingRun1);
    mvc.perform(put("/training-runs/{runId}", trainingRun1.getId())).andExpect(status().isOk());
  ...
  public void finishTrainingRunNotFound() throws Exception {
    MockHttpServletResponse response =
        mvc.perform(put("/training-runs/{runId}", nonExistentTrainingRunId))
            .andExpect(status().isNotFound())
            .andReturn()
            .getResponse();
    ApiEntityError error =
